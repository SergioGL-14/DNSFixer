...
function Do-FixStaleDNS {
    ...
    try {
        ...
        if ($diagnosticoTemp.Text -match "no corresponde a la red esperada") {
            ...
            try {
                Invoke-LocalOrRemote -Equipo $Equipo -Script { ipconfig /registerdns | Out-Null } | Out-Null
                Write-Log "[OK] Registro DNS forzado con 'ipconfig /registerdns'." $txtLog -LogType "ok"
            } catch {
                Write-Log "[ERROR] Fallo al ejecutar 'ipconfig /registerdns': $_" $txtLog -LogType "error"
                return
            }
            ...
            try {
                ...
                Write-Log "[ERROR] Fallo al limpiar la cache DNS: $_" $txtLog -LogType "error"
                return
            }
